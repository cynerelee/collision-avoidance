...
%     number=number+1;
%     t2(number)=t;
%     tt(number)=t2(number)/t1(number);
% end
% number=0;
% for t=1:1:301
%     number=number+1;
%     tt1(number)=(1+0.5*(number-1))/(1+0.01*(number-1));
% end
% m_tip_degree=xlsread('m_tip_degree.xlsx');
load('m_tip_degree.mat')
load('t1.mat')
Sample_Time=0.5;
for t=1:Sample_Time:151
    number=number+1
    s0_tra(number,:)=s0;
    figure(1);
    ...
    hold on;
%      m0=[mm(1)+10*sin((1+0.01*(t/0.5+1))*pi) mm(2)+10*sin((1+0.01*(t/0.5+1))*pi) mm(3)+sin((1+0.01*(t/0.5+1))*pi)];
%     m0=[mm(1)+10*sin(t1(number)*pi) mm(2)+10*cos(t1(number)*pi) mm(3)+sin(t1(number)*pi)];
     m0=[mm(1)+10*sin((1+0.01*(t/0.5+1))*pi) mm(2)+10*cos((1+0.01*(t/0.5+1))*pi) mm(3)+sin((1+0.01*(t/0.5+1))*pi)];
...
%     text(23,23,0,str);
    ...
    current_beta(number,:)=acos(dot(ez,n)/(norm(n)));
...
%     V=[0 0 0];
    ...
    imgPath = ['./image/' mat2str(number) '.png'];
    saveas(gcf,imgPath);
